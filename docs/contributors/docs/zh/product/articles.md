...
下文规则描述产品目标。当前后端使用单一作者、独立的源内容／快照／公开版本记录，
并有明确的 `UNPUBLISHED` 状态；待审核之外的状态允许软删除。
当前不能直接编辑已发布源内容，“保留旧公开版本的在线修订”尚未形成完整流程。
这些差异需要明确的产品决策，不应在维护任务中顺手改变。参见[当前实现](../architecture.md)。
